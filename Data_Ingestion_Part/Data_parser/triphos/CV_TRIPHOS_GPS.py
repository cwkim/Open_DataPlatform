...
""" 
    ...
    CV_TRIPHOS_GPS.py: TRIPHOS GPS 데이터 수집 코드
"""
...
class MongoDB_Default:
    def __init__(self, mo_address, mo_username, mo_password, mo_authsource, mo_database, mo_collection):
        ...
        # geo 딕셔너리 코드
        self.geo_list_code = {'서울특별시':0,'부산광역시':1,'대구광역시':2,'인천광역시':3,'광주광역시':4,'대전광역시':5,
                            '울산광역시':6,'세종특별자치시':7,'경기도':8,'강원도':9 ,'충청북도':10,'충청남도':11,
                            '전라북도':12,'전라남도':13,'경상북도':14,'경상남도':15,'제주도':16,'제주특별자치도':16} 
    ...
    def connectmongodb(self):
        ...
        # 데이터를 가져올 몽고 연결
        self.Bring_data_mo_client = MongoClient(self.mo_address,
        ...
        # 데이터를 저장할 몽고 연결
        self.Input_data_mo_client = MongoClient(self.save_mo_address + ':' +  self.save_mo_port,
    ...
    def triphos_GPS(self):
        ...
        for col in collection_list:
            ...
            #col = '201907'   # 테스트기 때문에 가장 데이터가 적은 19년 09월 데이터로 설정
            #print('test 컬렉션입니다.')
            if '' in col:
                year = 
                month = 
            ...
            #pipelines.append({'$match': {"$and": [{"GPS_LAT":{"$exists":1}}, {"GPS_LAT":{"$ne":0}}]}})
            #pipelines.append({'$group': {'_id': {'car_id':'$PHONE_NUM'}, 'avgGPS_LAT': { '$avg': "$GPS_LAT" }, 'avgGPS_LONG': { '$avg': "$GPS_LONG" }}})
            pipelines.append({'$group': {'_id': {'company_id': '$COMPANY_IDX', 'car_id':'$VEHICLE_IDX'}, 'first_GPS_LAT': { '$first': "$GPS_LAT" }, 'first_GPS_LONG': { '$first': "$GPS_LONG" }}})
            ...
            for j in range(len(car_gps_list)):
                try:
                    ...
                    gps_y = str(literal_eval(car_gps_list[j])['first_GPS_LAT'])
                    gps_x = str(literal_eval(car_gps_list[j])['first_GPS_LONG'])
                    url = ''+'x='+ gps_x + '&y=' + gps_y
                    header = {'authorization':''}
                    response = requests.get(url, headers=header)
                    geo_temp = json.loads(response.text).get('documents')[0]['region_1depth_name']

                    if geo_temp == '':
                        continue
                    else:
                        geo_list.append(json.loads(response.text).get('documents')[0]['region_1depth_name'])
            ...
            for key in geo_count.keys():
                ...
                self.input_data_dict = {'datetime':self.date_time_day,'geo_location':key,'geo_location_code':self.geo_list_code[key],'drive_count':geo_count[key]}               
                print('input_data_dict : ',self.input_data_dict)
                self.save_to_mongo('TRIPHOS_GPS_200401~(new)')
