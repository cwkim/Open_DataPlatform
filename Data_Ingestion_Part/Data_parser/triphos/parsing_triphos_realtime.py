...
"""
    ...
    parsing_triphos_realtime.py: continuously parsing Triphos log files to KETI MongoDB for Collection
"""
...
if __name__ == '__main__':
    logging.basicConfig(filename='parsing_triphos_realtime.log', level=logging.INFO,
    ...
    # Connect Triphos MongoDB
    src_client = mongodb_connect(url='', id='', pw='', initdb='')
    src_coll = src_client.get_database('').get_collection('')

    # Initial query
    import pdb; pdb.set_trace()
    # Match date: Year-Month-Day
    match_date = "^"+"20200227"
    for doc in src_coll.find({'RECORD_TIME_KOR':{'$regex':match_date}}).sort('_id', ASCENDING):
        if doc['RECORD_TIME_KOR'] is '20200312111059':
            print(doc)
            pdb.set_trace()

    for doc in src_coll.find().sort('_id', DESCENDING):
        print(doc) 
