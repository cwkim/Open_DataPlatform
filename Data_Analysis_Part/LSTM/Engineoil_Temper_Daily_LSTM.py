...
class Prediction_Temper_Realtime:
    def __init__(self, my_host, my_port, my_username, my_password, my_database):
        ...
        self.min_mean_engineoil = 7.225675824483157
        self.min_std_engineoil = 10.966491276620497

        self.mean_mean_engineoil = 48.79494249951156
        self.mean_std_engineoil = 28.250567814887802

        self.max_mean_engineoil = 85.61826842016106
        self.max_std_engineoil = 52.51136598396542
    ...
    def loadmodel(self):
        min_json_file = open("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_min_daily.json", "r")
        mean_json_file = open("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_mean_daily.json", "r")
        max_json_file = open("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_max_daily.json", "r")
        ...
        self.min_loadedmodel.load_weights("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_min_daily.h5")
        self.mean_loadedmodel.load_weights("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_mean_daily.h5")
        self.max_loadedmodel.load_weights("/home/wjchoi/github/keti_data_proc_develop/src/wjchoi/RMC_monitoring/LSTM/model/model_engineoil_max_daily.h5")
    ...
    def loaddata(self):
        ...
        query = """
            SELECT date, engineoil_mean, car_id, rnum FROM(
            	SELECT a.*,
        ...
        query = "SELECT date, engineoil_min, engineoil_mean, engineoil_max, car_id FROM DAILY_STATISTICS ORDER BY car_id desc, date;"
        print(query)
        ...
        self.df.columns = ['date', 'engineoil_min', 'engineoil_mean', 'engineoil_max', 'car_id']
    ...
    def znormalization(self):
        ...
        self.df['engineoil_min'] = self.df['engineoil_min'] - self.min_mean_engineoil
        self.df['engineoil_min'] = self.df['engineoil_min'] / self.min_std_engineoil

        self.df['engineoil_mmean'] = self.df['engineoil_mean'] - self.mean_mean_engineoil
        self.df['engineoil_mean'] = self.df['engineoil_mean'] / self.mean_std_engineoil

        self.df['engineoil_max'] = self.df['engineoil_max'] - self.max_mean_engineoil
        self.df['engineoil_max'] = self.df['engineoil_max'] / self.max_std_engineoil
    ...
    def dateprocessing(self):
        ...
        for car in self.carlist:
            ...
            min_temp_df = temp_df['engineoil_min'][-4:].to_numpy()
            mean_temp_df = temp_df['engineoil_mean'][-4:].to_numpy()
            max_temp_df = temp_df['engineoil_max'][-4:].to_numpy()
    ...
    def prediction(self):
        ...
        self.min_prediction = (min_prediction*self.min_std_engineoil)+ self.min_mean_engineoil
        self.mean_prediction = (mean_prediction * self.mean_std_engineoil) + self.mean_mean_engineoil
        self.max_prediction = (max_prediction * self.max_std_engineoil) + self.max_mean_engineoil
    ...
    def save(self):
        ...
        for i in range(len(self.min_prediction)):
            ...
            update_sql = 'UPDATE TEMPER_PREDICTION SET engineoil_pred = {engineoil_pred}, min_engineoil_pred = {min_engineoil_pred}, max_engineoil_pred = {max_engineoil_pred} '\
                        'WHERE date = \'{time}\' AND car_id = \'{car_id}\''.format(
                engineoil_pred = self.mean_prediction[i][0],
                min_engineoil_pred = self.min_prediction[i][0],
                max_engineoil_pred = self.max_prediction[i][0],
                time = datetime.now().date(),
                car_id = self.id_dataset[i]
            )

            print(update_sql)
            self.my_client.query(update_sql)
            print('저장 완료')
