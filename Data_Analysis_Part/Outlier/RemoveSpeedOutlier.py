...
if __name__ == '__main__':
    ...
    data = pd.concat([data, AOC_SPEED], axis=1)
    data.dropna(axis=0)
    ...
    gmmodel = joblib.load("outlier_gm_speed.pkl")

    #Gaussian Mixture Model Outlier detection
    np_dataset = np.array(result_dataset[['DRIVE_SPEED', 'AOC_SPEED']])
    ...
    plt.title('DRIVE_SPEED OUTLIER DETECTION (Gaussian Mixture)')
    plt.xlabel('DRIVE_SPEED', fontsize=10)
    plt.ylabel('AOC_DRIVE_SPEED', fontsize=10)
    ...
    #print('../{}/{}.txt'.format(token, filename_str))

    '''
    file = open('../{}/{}.txt'.format(token,filename_str), 'w')
    file.write('주행속도 이상치 탐지 결과 요약')
    ...
    file.write('\n이상치 제거 그림: {}_img.html'.format(filename_str))
    ...
    html_text += '\n<html>'
    html_text += '\n<head>'
    html_text += '\n    <title>주행속도 이상치 탐지 결과</title>'
    html_text += '\n    <meta charset="utf-8">'
    html_text += '\n</head>'
    html_text += '\n'
    html_text += '\n<body>'
    html_text += '\n<h1> 주행속도 이상치 탐지 결과 </h1>'
    html_text += '\n<p>'
    html_text += '\n데이터 수: '+ str(len(result_dataset['GPS_LAT'])) + '<br>'
    ...
    html_text += '\n이상치 제거 파일: '
    html_text += '\n<A href = "result_1.csv" target="blank"> result_1.csv</A>' + '<br>'
    html_text += '\n이상치 제거 그림: '
    html_text += '\n<A href = "result_1.png" target="blank"> result_1.png</A>' + '<br>'
