...
def _getse(s_ut, e_ut, _dictbuf_list, _sec, meta):
    ...
    DSkeylist = total_dict['DRIVE_SPEED'].keys()
    DSkeylist.sort()
    print(len(DSkeylist))
    start=-1
    end=-1
    for i in range(len(DSkeylist)-1):
        ...
        if int(DSkeylist[i+1]) - int(DSkeylist[i]) >= 600 or i+1 == len(DSkeylist)-1:
            ...
            if end-start >= 600:
                ...
                if _check1 or _check2:
                    ...
                    continue
                # 2km 이상 주행할 경우 주행으로 판단
                if end_dis - start_dis > 1 and start_dis != 0 and end_dis != 0:
