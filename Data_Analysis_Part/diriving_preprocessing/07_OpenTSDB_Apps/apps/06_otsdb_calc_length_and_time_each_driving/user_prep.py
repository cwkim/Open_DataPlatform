...
def _driving(s_ut, e_ut, _dictbuf_list, _sec, meta):
    ...
    for i in range(len(total_dict['DRIVE_LENGTH_TOTAL_1'])):
        ...
        if int(current_time) - int(before_time) > 1:
            put_dps_dict1[str(before_time)] = int(total_dict['DRIVE_LENGTH_TOTAL_1'][i-1][1]) - int(start_length)
            put_dps_dict2[str(before_time)] = int(total_dict['DRIVE_LENGTH_TOTAL_1'][i-1][0]) - int(start_time)
            start_time = total_dict['DRIVE_LENGTH_TOTAL_1'][i][0]
            start_length = total_dict['DRIVE_LENGTH_TOTAL_1'][i][1]
        
        if len(total_dict['DRIVE_LENGTH_TOTAL_1'])-1 == i:
            put_dps_dict1[str(before_time)] = int(total_dict['DRIVE_LENGTH_TOTAL_1'][i][1]) - int(start_length)
            put_dps_dict2[str(before_time)] = int(total_dict['DRIVE_LENGTH_TOTAL_1'][i][0]) - int(start_time)

    tags_dict1 = { 'VEHICLE_NUM' : str(copied_ori_dict1['tags']['VEHICLE_NUM']), 'fieldname' : 'DRIVE_LENGTH' }
    tags_dict2 = { 'VEHICLE_NUM' : str(copied_ori_dict1['tags']['VEHICLE_NUM']), 'fieldname' : 'DRIVE_TIME' }
