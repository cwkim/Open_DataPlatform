...
def _rm_outlier(s_ut, e_ut, _dictbuf_list, _sec, meta):
    ...
    if nowidx < len(total_dict["DRIVE_LENGTH_TOTAL"]):
        while True:
            ...
            tsdif = int(total_dict['DRIVE_LENGTH_TOTAL'][nextidx][0]) - int(
                total_dict['DRIVE_LENGTH_TOTAL'][nowidx][0])
    ...
    flag=0
    #속도가 0보다 작거나 260보다 클 경우 제거
    #속도가 1초 기준으로 13km/h 감소하거나 증가 할 경우 flag값을 1로 세팅 후 제거,  flag가 1일 경우 속도가 0인 값들 제거
    for i in range(len(total_dict['DRIVE_SPEED'])):
        if i!=0:
            _accel=(total_dict['DRIVE_SPEED'][i][1]-total_dict['DRIVE_SPEED'][pre_idx][1])/(int(total_dict['DRIVE_SPEED'][i][0])-int(total_dict['DRIVE_SPEED'][pre_idx][0]))
            pre_idx=i
            val=total_dict['DRIVE_SPEED'][i][1]
            if val >= 0 and val <= 260:
                if flag > 0 and val==0:
                    continue
                if _accel < 14 and _accel > -14:
                    flag=0
                    put_dps_dict2[total_dict['DRIVE_SPEED'][i][0]]=val
                else:
                    flag=1
        else:
            put_dps_dict2[total_dict['DRIVE_SPEED'][i][0]]=total_dict['DRIVE_SPEED'][i][1]
