...
if __name__ =='__main__':
    ...
    for collection_name in collection_list:
        ...
        for Vechile_num in carid_list:
            ...
            cursor, collection_total_line = MongoDB_query(ip, port, username, password, db_name, Vechile_num, 0, collection_name)
            ...
            try:
                if not os.path.isdir(CSV_DIR + "/" + collection_name + "/"):
                    os.makedirs(os.path.join(CSV_DIR + "/" + collection_name + "/"))
            except OSError as e:
                print("Failed to create directory!!!")
                raise
            df.to_csv(CSV_DIR + "/" + collection_name + "/%s.csv" %str(Vechile_num), mode='w', encoding='utf-8')
