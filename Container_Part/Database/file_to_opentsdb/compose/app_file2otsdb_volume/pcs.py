...
def _consume(works_to_do, _empty1, meta, _empty2):
    ...
    while True:
        ...
        if works == "END":
            ...
            try:
                response = sess.post("http://172.20.0.2:60010/api/put", data=json.dumps(works), headers=headers)
                ...
                while response.status_code > 204:
                    ...
                    response = sess.post("http://172.20.0.2:60010/api/put", data=json.dumps(works), headers=headers)  # , timeout=10)
                    tries += 1
...
class Workers:
    ...
    def start_work(self, meta):
        ...
        if self.nP != 0:
            ...
            self.producers.apply(_produce, self.pwork_basket_list, self.cwork_basket, meta)
