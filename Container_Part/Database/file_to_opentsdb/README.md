...
- OpenTSDB로 입력하고자 하는 데이터는 (로컬호스트 파일시스템에 위치)  
  - 전송, 저장을 위한 파일은 특정 directory 내부에 위치시킴
...
## 설치 및 동작 테스트 
  ...
  2. file_to_opentsdb compose 디렉토리로 이동
            ...
            $ cd Docker/sample/file_to_opentsdb/compose
  ...
  4. docker-compose로 container 실행
        ...
        $ sudo docker-compose up -d app_csv_to_opentsdb # csv or xlsx data put 컨테이너 실행
...
## 컨테이너 실행 후 로그 확인
    $ sudo docker logs -f app_csv_to_opentsdb  # windows or linux 환경
...
## 코드 수정 및 수정한 코드 실행(tsdb에 입력된 field이외에 다른 field를 추가로 입력할 때)
  1. 공유 디렉토리로 이동
         ...
         $ cd Docker/sample/file_to_opentsdb/compose/app_file2otsdb_volume
